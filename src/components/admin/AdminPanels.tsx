...
export function ApiKeyVaultPanel() {
  ...
  async function addKey(e: React.FormEvent) {
    ...
    setMsg(data.test?.status === "online" ? `✅ ${form.label} — ONLINE` : `⚠️ ${form.label} saved — ${data.test?.message || "test pending"}`);
  ...
  async function testKey(id: string) {
    ...
    const data = await res.json();
    setMsg(data.status === "online" ? "✅ Key is online" : `❌ ${data.message}`);
    load();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Total Keys", value: stats.total, color: "text-blue-400" },
      ...
      <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.04] to-transparent p-6">
        ...
        {showAdvanced && <form onSubmit={addKey} className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="Label (e.g. Primary OpenAI, Backup Key)" required className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm" />
          <input value={form.keyValue} onChange={(e) => setForm({ ...form, keyValue: e.target.value })} placeholder="Paste API key here..." required type="password" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 font-mono text-sm md:col-span-2" />
      ...
      {grouped.map((group) => (
        <div key={group.id} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <h3 className="font-semibold">{group.icon} {group.name} <span className="text-xs text-neutral-600">({group.keys.length} keys)</span></h3>
          {group.keys.length === 0 ? (
            <p className="mt-2 text-xs text-neutral-600">No keys yet — add above</p>
          ) : (
                ...
                <div key={k.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white/5 px-4 py-3">
                  <div className="flex-1">
                    ...
                    <p className="mt-0.5 font-mono text-xs text-neutral-600">
                      {showKey[k.id] ? k.keyValue : k.keyValue ? "••••••••" + k.keyValue.slice(-4) : "empty"}
                      {" · "}Priority: {k.priority}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => setShowKey({ ...showKey, [k.id]: !showKey[k.id] })} className="rounded-lg border border-white/10 px-2 py-1 text-[10px]">👁</button>
                    <button onClick={() => testKey(k.id)} className="rounded-lg border border-white/10 px-2 py-1 text-[10px]">Test</button>
                    <button onClick={() => toggleActive(k.id, k.isActive)} className="rounded-lg border border-white/10 px-2 py-1 text-[10px]">{k.isActive ? "Disable" : "Enable"}</button>
                    <button onClick={() => deleteKey(k.id)} className="rounded-lg border border-red-500/30 px-2 py-1 text-[10px] text-red-400">Delete</button>
                  </div>
                </div>
              ))}
            </div>
...
export function ShowcaseAdminPanel() {
  ...
  const load = () => fetch("/api/showcase").then((r) => r.json()).then((d) => setItems(d.items || []));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-white/10 p-6">
        ...
        <p className="text-sm text-neutral-500">Landing page videos, images, ads — dynamic gallery</p>
        <form onSubmit={add} className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <input value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })} placeholder="Badge (optional)" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm" />
          <input value={form.mediaUrl} onChange={(e) => setForm({ ...form, mediaUrl: e.target.value })} placeholder="Media URL (image or video)" required className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm md:col-span-2" />
          <input value={form.subtitle} onChange={(e) => setForm({ ...form, subtitle: e.target.value })} placeholder="Subtitle / description" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm md:col-span-2" />
          <button type="submit" className="rounded-xl bg-orange-500 px-4 py-3 text-sm font-semibold md:col-span-2">+ Add to Showcase</button>
        </form>
      </div>
          ...
          <div key={item.id} className="rounded-xl border border-white/10 overflow-hidden">
            ...
            <div className="p-3 flex justify-between">
              <div>
                ...
                <p className="text-[10px] text-neutral-600">{item.category} · {item.type}</p>
              </div>
              <button onClick={() => del(item.id)} className="text-xs text-red-400">Delete</button>
            </div>
          </div>
        ))}
      </div>
